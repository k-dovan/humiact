...
save_npy_path = 'dataset-NTU-RGB-D/nturgb+d_npy/'
load_txt_path = 'dataset-NTU-RGB-D/nturgb+d_skeletons/'
missing_file_path = 'dataset-NTU-RGB-D/NTU_RGBD_samples_with_missing_skeletons.txt'
...
def _read_skeleton(file_path):
    #
    # read .skeleton file and save RGB X,Y
    #
    f = open(file_path, 'r')
    datas = f.readlines()
    f.close()
    ...
    # specify the maximum number of the body shown in the sequence, according to
    # the certain sequence, need to pune the abundant bodies.
    ...
    # Edit-save data of all rgb_bodys on an ndarray intead
    bodymat['rgb_bodys'] = np.zeros(shape=(max_body, nframe, njoints, 2))
    ...
    for frame in range(nframe):
        ...
        for body in range(bodycount):
            ...
            for joint in range(njoints):
                ...
                # if save_rgbxy:
                bodymat['rgb_bodys'][body, frame,joint] = jointinfo[5:7]

    # don't save the data if exist only one body in all frames
    if max(bodymat['nbodys']) < 2:
        return dict() # return empty dict

    # prune the redundant bodys
    ...
    # Edit-prune redundant bodys
    bodymat['rgb_bodys'] = bodymat['rgb_bodys'][:max(bodymat['nbodys'])]
...
if __name__ == '__main__':
    ...
    for ind, each in enumerate(datalist):
        ...
        # only save if exist more than one body in any frame
        if mat:
            mat = np.array(mat)
